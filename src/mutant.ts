...
export class Mutant {
  constructor(
    public id: number,
    public original: types.Node,
    public replacement: types.Node,
  ...
  private getMutationLineIndexes(originalText: string): [number, number] {
    ...
    while (
      endIndexLines < originalText.length &&
      !isLineBreak(originalText.charCodeAt(endIndexLines))
    ) {
      endIndexLines++;
    }
  ...
  public mutatedLines(originalText: string): string {
    const [startIndex, endIndex] = this.getMutationLineIndexes(originalText);
    return (
      originalText.substring(startIndex, this.original.start!) +
      generate(this.replacement).code +
      originalText.substring(this.original.end!, endIndex)
    );
  }
}
