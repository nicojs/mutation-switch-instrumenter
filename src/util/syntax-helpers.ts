...
export function mutantTestExpression(mutantId: number): types.BinaryExpression {
  return types.binaryExpression(
    '===',
    memberExpressionChain(GLOBAL, ACTIVE_MUTANT),
    types.numericLiteral(mutantId)
  );
}
 ...
 */
export function memberExpressionChain(
  ...identifiers: (string | number)[]
): types.Identifier | types.MemberExpression {
...
export function mutationCoverageSequenceExpression(
  mutants: Mutant[],
  targetExpression?: types.Expression
): types.Expression {
...
export function offsetLocations(
  file: types.File,
  { position, line, column }: { position: number; line: number; column: number }
): void {
...
export function createMutatedAst<T extends types.Node>(
  contextPath: NodePath<T>,
  mutant: Mutant
): T {
